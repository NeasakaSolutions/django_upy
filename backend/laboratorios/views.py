...
class Clase2(APIView):
    ...
    def put(self, request, id):
        ...
        if 'foto' in request.FILES:
            try:
                ...
                #if anterior and os.path.exists(f"./uploads/laboratorios/{anterior}"):
                    #os.remove(f"./uploads/laboratorios/{anterior}")

                if anterior:
                    anterior_path = os.path.join(settings.MEDIA_ROOT, "laboratorios", anterior)
    ...
    def delete(self, request, id):
        ...
        # Borrar foto de la carpeta solo si existe
        #if data.foto and os.path.exists(f"./uploads/laboratorios/{data.foto}"):
            #os.remove(f"./uploads/laboratorios/{data.foto}")

        if data.foto:
            foto_path = os.path.join(settings.MEDIA_ROOT, "laboratorios", data.foto)
